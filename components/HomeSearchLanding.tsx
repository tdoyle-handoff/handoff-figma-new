...
export default function HomeSearchLanding() {
  ...
  const generatePDFContent = () => {
    ...
    let content = `HOME BUYER PROFILE\n\n`;
    content += `Generated on: ${new Date().toLocaleDateString()}\n\n`;

    // Contact Information
    content += `CONTACT INFORMATION\n`;
    content += `Name: ${onboardingData.fullName}\n`;
    content += `Phone: ${onboardingData.phone}\n`;
    content += `Date of Birth: ${onboardingData.dateOfBirth}\n`;
    content += `Gender: ${onboardingData.gender}\n`;
    content += `Preferred Contact: ${onboardingData.preferredContact}\n\n`;

    // Budget & Financing
    content += `BUDGET & FINANCING\n`;
    content += `Max Price: $${onboardingData.maxPrice?.toLocaleString()}\n`;
    content += `Financing Type: ${onboardingData.financingType}\n`;
    content += `Down Payment: ${onboardingData.downPaymentPercent}%\n`;
    content += `Monthly PITI Comfort: $${onboardingData.comfortMonthlyPiti?.toLocaleString()}\n`;
    content += `Pre-approved: ${onboardingData.preApproved ? 'Yes' : 'No'}\n`;
    if (onboardingData.lenderName) content += `Lender: ${onboardingData.lenderName}\n`;
    content += `\n`;

    // Property Criteria
    content += `PROPERTY CRITERIA\n`;
    content += `Min Bedrooms: ${onboardingData.minBeds}\n`;
    content += `Min Bathrooms: ${onboardingData.minBaths}\n`;
    content += `Min Square Footage: ${onboardingData.minSqft}\n`;
    content += `Property Types: ${onboardingData.propertyTypes?.join(', ')}\n`;
    content += `Parking: ${onboardingData.parking}\n\n`;

    // Location Preferences
    content += `LOCATION & LIFESTYLE\n`;
    content += `Max Commute: ${onboardingData.maxCommute} minutes\n`;
    content += `School Needs: ${onboardingData.schoolNeeds}\n`;
    content += `Transit Needs: ${onboardingData.transitNeeds}\n`;
    content += `Pets: ${onboardingData.pets ? 'Yes' : 'No'}\n\n`;

    // Goals & Timeline
    content += `GOALS & TIMELINE\n`;
    content += `Why Buy Now: ${onboardingData.whyBuyNow}\n`;
    content += `Target Move-in: ${onboardingData.targetMoveIn}\n`;
    content += `Hard Deadlines: ${onboardingData.hardDeadlines}\n\n`;

    // Additional preferences
    if (onboardingData.layoutMustHaves) {
      content += `LAYOUT MUST-HAVES\n${onboardingData.layoutMustHaves}\n\n`;
    }

    if (onboardingData.dealbreakers) {
      content += `DEALBREAKERS\n${onboardingData.dealbreakers}\n\n`;
    }

    if (onboardingData.additionalNotes) {
  ...
  const downloadPDF = () => {
    try {
      ...
      link.href = url;
      link.download = `home-buyer-profile-${onboardingData?.fullName?.replace(/\s+/g, '-') || 'profile'}-${new Date().toISOString().split('T')[0]}.txt`;
  ...
  const sendEmail = async () => {
    try {
      const content = generatePDFContent();
      const subject = `Home Buyer Profile - ${onboardingData?.fullName}`;
      const body = `Hi there,\n\nPlease find my completed home buyer profile below:\n\n${content}\n\nBest regards,\n${onboardingData?.fullName}`;
